...
- [ ] **PRIMÁRIO — leitura de selo das pranchas** (arquitetura decidida via mapa do extract-stamp):
  ...
  - [x] **Painel "Selos das pranchas"** no NexoWorkspace (SelosPanel): botão "Ler pranchas" → tabela folha/descrição/disciplina/confiança. **FALTA teste ao vivo no browser** (canvas + OCR real).
